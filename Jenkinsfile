...
private void buildInsideNode(map)
{
    ...
    if (map.deploy) {
        ...
        stage('Start Scale Set Upgrade'){
          sh '''
            az vmss extension set --resource-group jaywindsor-java-scenario-1 --vmss-name xwiki-ubuntu --name CustomScript --publisher Microsoft.Azure.Extensions --version 2.0 --force-update
            az vmss wait --updated --resource-group jaywindsor-java-scenario-1 --name xwiki-ubuntu
            az vmss rolling-upgrade start --resource-group jaywindsor-java-scenario-1 --name xwiki-ubuntu
          '''
        }
        stage('AZ Logout'){
          sh '''
